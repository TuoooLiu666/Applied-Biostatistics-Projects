...
# dat %>% group_by(SEX,CURSMOKE) %>% mutate(n=n()) %>% 
...
DataExplorer::plot_qq(dat[,c("AGE", "BMI", "SYSBP", "DIABP", "TOTCHOL")])
...
table <- arsenal::tableby(interaction(PERIOD, CVD) ~  SEX + CURSMOKE + CIGPDAY + AGE + BMI + SYSBP + DIABP + TOTCHOL, data = dat, control = my_controls)
