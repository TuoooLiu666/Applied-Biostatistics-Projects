...
```{r}
data_wide <- tidyr::spread(data %>% select(1:3,6), key = day, value = MHOWS) %>% select(1:2, 5:10) %>% filter(!PATIENT %in% c(28482, 930174)) %>% mutate(treat=factor(treat))

# calculate sample variance-covariance matrix
cov(data_wide[,3:8], use = "complete.obs")
```

##### Mixed-MI: categorical time
```{r}
