...
data$age <- ifelse(data$RIDAGEYR<19, 1, ifelse(data$RIDAGEYR<35, 2, ifelse(data$RIDAGEYR<65, 3, 4)))
data$tobacco <- ifelse(data$SMQ720 %in% c(0:5), "light", ifelse(data$SMQ720 %in% c(6:10), "moderate", ifelse(data$SMQ720 %in% c(11:40), "heavy", NA)))
...
# adjusted
model_adjusted <- svyglm(LBXBPB~SMQ905 + RIAGENDR + RIDAGEYR+ BMXBMI + educ + inc + alc + merit, design = design_m)
...
model_l <- svyglm(LBXBPB~SMQ905 + RIAGENDR + RIDAGEYR + BMXBMI + inc, design = design_l)
sum_l <- model_l %>% tbl_regression()

design_m1 <- subset(design_m, tobacco=="moderate")
model_m1 <- svyglm(LBXBPB~SMQ905 + RIAGENDR + RIDAGEYR + BMXBMI + inc, design = design_m1)
sum_m1 <- model_m1 %>% tbl_regression()

design_h <- subset(design_m, tobacco=="heavy")
model_h <- svyglm(LBXBPB~SMQ905 + RIAGENDR + RIDAGEYR + BMXBMI + inc, design = design_h)
...
### sensitivity analysis using age groups
```{r}
# sensitivity analysis
model_sen <- svyglm(LBXBPB~SMQ905 + RIAGENDR + age + BMXBMI + inc, design = design_m)
