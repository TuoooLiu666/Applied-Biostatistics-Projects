...
svyhist(~RIDAGEYR, design=design_m, na.rm=F)
...
model_adjusted <- svyglm(LBXBPB~ SMQ905 + RIAGENDR + RIDAGEYR + BMXBMI + educ + alc + merit, design = design_m)
...
model_final <- svyglm(LBXBPB~SMQ905+RIDAGEYR+RIAGENDR+RIDRETH3+BMXBMI+educ, design = design_m)
sum_e <- model_final %>%  tbl_regression()
sum_e

# final model
model_final1 <- svyglm(LBXBPB~SMQ905+RIDAGEYR+RIAGENDR+BMXBMI+educ, design = design_m)
sum_f <- model_final1 %>%  tbl_regression()
sum_f
```
...
model_sen <- svyglm(LBXBPB~ecig*age_65, design = design_m)
sum_i <- model_sen %>%  tbl_regression()

# age >= 65
design_65 <- subset(design_m, age_65=="65+")
model_age65 <- svyglm(LBXBPB~ecig + RIAGENDR + RIDAGEYR+ BMXBMI + educ, design = design_65)
sum_age1 <- model_age65 %>% tbl_regression()

# age < 65
design_64 <- subset(design_m, age_65=="<65")
model_age64 <- svyglm(LBXBPB~ecig + RIAGENDR + RIDAGEYR + BMXBMI + educ, design = design_64)
