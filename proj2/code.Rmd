...
```{r}
design_s <- svydesign(id=data$SEQN, strata=data$SDMVSTRA, weights = data$WTINT2YR, data=data, nest=T)
# subset design to get rid of NA
design1 <- subset(design_s, SMQ900!="Don't know" & is.na(SMQ900)==F)

# age among vaping & non-vaping
svyby(~RIDAGEYR, ~SMQ900, design1, svymean, na.rm = T)

# Num. of cigarette/day among vaping & non-vaping
svyby(~SMQ720, ~SMQ900, design1, svymean, na.rm = T)

# gender/race/tobacco use/alcohol use
design1 %>%
  tbl_svysummary(by = SMQ900, percent = "col", include = c(RIAGENDR, RIDRETH3,SMQ040,ALQ151))
```
```{r}
design_m <- svydesign(id=data$SEQN, strata=data$SDMVSTRA, weights = data$WTMEC2YR, data=data, nest=T)
# subset design to get rid of NA
design2 <- subset(design_m, SMQ900!="Don't know" & is.na(SMQ900)==F)

# BMI among vaping & non-vaping
svyby(~BMXBMI, ~SMQ900, design2, svymean, na.rm = T)
# Bood lead level among vaping & non-vaping
svyby(~LBXBPB, ~SMQ900, design2, svymean, na.rm = T)
```
